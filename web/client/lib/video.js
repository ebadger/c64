...
// paints it at the native framebuffer resolution. CSS scales both timing profiles into one stable
// display viewport with crisp pixel edges. Presentation only — never touches machine state (see
...
export const C64_PRESENTATION_SIZE = Object.freeze({ width: 384, height: 284 });

export class CanvasRenderer {
  constructor(canvas) {
    this._canvas = canvas;
    this._ctx = canvas.getContext("2d", { alpha: false });
    this._canvas.style.aspectRatio =
      `${C64_PRESENTATION_SIZE.width} / ${C64_PRESENTATION_SIZE.height}`;
    this._width = 0;
    this._height = 0;
    this._image = null;
  ...
  draw(frame) {
    ...
    if (
      frame.sequence === this._lastSequence
