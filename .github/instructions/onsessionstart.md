...
- `docs/LEARNINGS.md` — **canonical** workflow rules (§1–7) + the Tier-1 lessons digest
...
- `.template-source` — canonical template provenance + last reviewed upstream commit.
...
The `compliance-hooks` extension checks the canonical operating-system template
automatically and appends its result to this instruction. If that result is absent, run:

```
node scripts/dev/review-template-updates.mjs check
```

If updates are reported, read `specs/TEMPLATE-INHERITANCE.md` before changing inherited
operating files. The check fails open offline and does not block urgent product work.

## Core Rules — canonical in `docs/LEARNINGS.md` §1–7 (don't re-derive)
...
7. **Reconcile template changes deliberately** — adopt/adapt/defer/not-applicable; never
   merge upstream wholesale or advance `.template-source` before every change is accounted for. (§7)
8. **When in doubt about permissions, ask** rather than assume.
